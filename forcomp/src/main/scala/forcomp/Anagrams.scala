...
object Anagrams {
  ...
  /** Converts the word into its character occurence list.
    ...
    */
  def foldOp(o: (Char, Int), ocs: Occurrences): Occurrences
  = ocs match {
    case Nil => o :: Nil
    case (chr, n) :: xs => if (o._1 == chr) foldOp((o._1, n + o._2), xs)
    else o :: foldOp((chr, n), xs)
  }

  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.sorted.map((c: Char) => (c, 1)).foldRight[Occurrences](Nil)(foldOp)
  }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences =
    s.flatMap(wordOccurrences).sortBy(_._1).foldRight(List.empty[(Char, Int)])(foldOp)
